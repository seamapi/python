...
@dataclass
class AcsUserAccessSchedule(ResourceMapping):
...
@dataclass
class AcsUserErrors(ResourceMapping):
...
@dataclass
class AcsUserFrom(ResourceMapping):
...
@dataclass
class AcsUserTo(ResourceMapping):
...
@dataclass
class AcsUserPendingMutations(ResourceMapping):
    ...
    def from_dict(cls, d: Dict[str, Any]):
        return cls(
            ...
            from_=(
                AcsUserFrom.from_dict(d.get("from"))
                if d.get("from") is not None
                else None
            ),
            to=AcsUserTo.from_dict(d.get("to")) if d.get("to") is not None else None,
...
@dataclass
class AcsUserSaltoKsMetadata(ResourceMapping):
...
@dataclass
class AcsUserSaltoSpaceMetadata(ResourceMapping):
...
@dataclass
class AcsUserWarnings(ResourceMapping):
...
class AcsUser:
    ...
    access_schedule: AcsUserAccessSchedule
    acs_system_id: str
    acs_user_id: str
    ...
    is_managed: bool
    is_suspended: bool
    pending_mutations: List[AcsUserPendingMutations]
    phone_number: str
    salto_ks_metadata: AcsUserSaltoKsMetadata
    salto_space_metadata: AcsUserSaltoSpaceMetadata
    user_identity_email_address: str
    user_identity_full_name: str
    user_identity_id: str
    user_identity_phone_number: str
    warnings: List[AcsUserWarnings]
    workspace_id: str
    ...
    def from_dict(cls, d: Dict[str, Any]):
        return cls(
            access_schedule=(
                AcsUserAccessSchedule.from_dict(d.get("access_schedule"))
            ...
            email_address=d.get("email_address", None),
            errors=[AcsUserErrors.from_dict(i) for i in d.get("errors") or []],
            ...
            pending_mutations=[
                AcsUserPendingMutations.from_dict(i)
                for i in d.get("pending_mutations") or []
            ],
            phone_number=d.get("phone_number", None),
            salto_ks_metadata=(
                AcsUserSaltoKsMetadata.from_dict(d.get("salto_ks_metadata"))
                if d.get("salto_ks_metadata") is not None
                else None
            ),
            salto_space_metadata=(
                AcsUserSaltoSpaceMetadata.from_dict(d.get("salto_space_metadata"))
            ...
            warnings=[AcsUserWarnings.from_dict(i) for i in d.get("warnings") or []],
