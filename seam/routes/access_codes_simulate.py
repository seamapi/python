...
class AbstractAccessCodesSimulate(abc.ABC):
    ...
    @abc.abstractmethod
    def create_unmanaged_access_code(self, *, code: str, device_id: str, name: str) -> UnmanagedAccessCode:
...
class AccessCodesSimulate(AbstractAccessCodesSimulate):
    ...
    @route_metadata(path="/access_codes/simulate/create_unmanaged_access_code", has_required_parameters=True, has_pagination=False)
    def create_unmanaged_access_code(self, *, code: str, device_id: str, name: str) -> UnmanagedAccessCode:
        """Simulates the creation of an `unmanaged access code <https://docs.seam.co/low-level-apis/smart-locks/access-codes/migrating-existing-access-codes>`_ in a `sandbox workspace <https://docs.seam.co/core-concepts/workspaces#sandbox-workspaces>`_.
        ...
        if not json_payload:
            raise ValueError("At least one parameter is required for /access_codes/simulate/create_unmanaged_access_code")

        res = self.client.post("/access_codes/simulate/create_unmanaged_access_code", json=json_payload)
