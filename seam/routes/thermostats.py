...
from .thermostats_daily_programs import (
    ...
    AbstractAsyncThermostatsDailyPrograms,
    AsyncThermostatsDailyPrograms,
)
from .thermostats_schedules import (
    AbstractThermostatsSchedules,
    ThermostatsSchedules,
    AbstractAsyncThermostatsSchedules,
    AsyncThermostatsSchedules,
)
from .thermostats_simulate import (
    AbstractThermostatsSimulate,
    ThermostatsSimulate,
    AbstractAsyncThermostatsSimulate,
    AsyncThermostatsSimulate,
)
...
class AbstractAsyncThermostats(abc.ABC):

    @property
    @abc.abstractmethod
    def daily_programs(self) -> AbstractAsyncThermostatsDailyPrograms:
        raise NotImplementedError()

    @property
    @abc.abstractmethod
    def schedules(self) -> AbstractAsyncThermostatsSchedules:
        raise NotImplementedError()

    @property
    @abc.abstractmethod
    def simulate(self) -> AbstractAsyncThermostatsSimulate:
        raise NotImplementedError()
...
class AsyncThermostats(AbstractAsyncThermostats):
    def __init__(self, client: AsyncSeamHttpClient, defaults: Dict[str, Any]):
        self.client = client
        self.defaults = defaults
        self._daily_programs = AsyncThermostatsDailyPrograms(
            client=client, defaults=defaults
        )
        self._schedules = AsyncThermostatsSchedules(client=client, defaults=defaults)
        self._simulate = AsyncThermostatsSimulate(client=client, defaults=defaults)

    @property
    def daily_programs(self) -> AsyncThermostatsDailyPrograms:
        return self._daily_programs

    @property
    def schedules(self) -> AsyncThermostatsSchedules:
        return self._schedules

    @property
    def simulate(self) -> AsyncThermostatsSimulate:
        return self._simulate
