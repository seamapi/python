...
class AbstractAsyncConnectedAccounts(abc.ABC):

    @property
    @abc.abstractmethod
    def simulate(self) -> AbstractAsyncConnectedAccountsSimulate:
        raise NotImplementedError()
...
class AsyncConnectedAccounts(AbstractAsyncConnectedAccounts):
    def __init__(self, client: AsyncSeamHttpClient, defaults: Dict[str, Any]):
        self.client = client
        self.defaults = defaults
        self._simulate = AsyncConnectedAccountsSimulate(
            client=client, defaults=defaults
        )

    @property
    def simulate(self) -> AsyncConnectedAccountsSimulate:
        return self._simulate
