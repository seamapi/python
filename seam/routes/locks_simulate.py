...
class AbstractLocksSimulate(abc.ABC):
    ...
    @abc.abstractmethod
    def keypad_code_entry(self, *, code: str, device_id: str, wait_for_action_attempt: Optional[Union[bool, Dict[str, float]]] = None) -> ActionAttempt:
    ...
    @abc.abstractmethod
    def manual_lock_via_keypad(self, *, device_id: str, wait_for_action_attempt: Optional[Union[bool, Dict[str, float]]] = None) -> ActionAttempt:
...
class LocksSimulate(AbstractLocksSimulate):
    ...
    @route_metadata(path="/locks/simulate/keypad_code_entry", has_required_parameters=True, has_pagination=False)
    def keypad_code_entry(self, *, code: str, device_id: str, wait_for_action_attempt: Optional[Union[bool, Dict[str, float]]] = None) -> ActionAttempt:
        """Simulates the entry of a code on a keypad. You can only perform this action for `August <https://docs.seam.co/device-and-system-integration-guides/august-locks>`_ devices within `sandbox workspaces <https://docs.seam.co/core-concepts/workspaces#sandbox-workspaces>`_.
        ...
        if not json_payload:
            raise ValueError("At least one parameter is required for /locks/simulate/keypad_code_entry")
    ...
    @route_metadata(path="/locks/simulate/manual_lock_via_keypad", has_required_parameters=True, has_pagination=False)
    def manual_lock_via_keypad(self, *, device_id: str, wait_for_action_attempt: Optional[Union[bool, Dict[str, float]]] = None) -> ActionAttempt:
        """Simulates a manual lock action using a keypad. You can only perform this action for `August <https://docs.seam.co/device-and-system-integration-guides/august-locks>`_ devices within `sandbox workspaces <https://docs.seam.co/core-concepts/workspaces#sandbox-workspaces>`_.
        ...
        res = self.client.post("/locks/simulate/manual_lock_via_keypad", json=json_payload)
