...
class AbstractAsyncAccessGrants(abc.ABC):

    @property
    @abc.abstractmethod
    def unmanaged(self) -> AbstractAsyncAccessGrantsUnmanaged:
        raise NotImplementedError()
...
class AsyncAccessGrants(AbstractAsyncAccessGrants):
    def __init__(self, client: AsyncSeamHttpClient, defaults: Dict[str, Any]):
        self.client = client
        self.defaults = defaults
        self._unmanaged = AsyncAccessGrantsUnmanaged(client=client, defaults=defaults)

    @property
    def unmanaged(self) -> AsyncAccessGrantsUnmanaged:
        return self._unmanaged
    ...
    async def update(
        ...
        *,
        ...
        await self.client.patch("/access_grants/update", json=json_payload)
