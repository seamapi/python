...
from .phones_simulate import (
    AbstractPhonesSimulate,
    PhonesSimulate,
...
class AbstractAsyncPhones(abc.ABC):

    @property
    @abc.abstractmethod
    def simulate(self) -> AbstractAsyncPhonesSimulate:
        raise NotImplementedError()
...
class AsyncPhones(AbstractAsyncPhones):
    def __init__(self, client: AsyncSeamHttpClient, defaults: Dict[str, Any]):
        self.client = client
        self.defaults = defaults
        self._simulate = AsyncPhonesSimulate(client=client, defaults=defaults)

    @property
    def simulate(self) -> AsyncPhonesSimulate:
        return self._simulate
