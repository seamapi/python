...
@pytest.mark.asyncio
async def test_seam_client_from_api_key_returns_instance_authorized_with_api_key(
    server,
):
...
@pytest.mark.asyncio
async def test_seam_client_constructor_returns_instance_authorized_with_api_key(
    server,
):
    endpoint, seed = server
